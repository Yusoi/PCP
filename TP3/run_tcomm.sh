...
array_matrix_size="4096"
array_num_machines="8"
...
do
    ...
    for i in $array_num_machines; do
	    export N_MACHINES=$i
	    export C_FILE=tp2tcomm
	    export FILENAME=tp2tcomm
	    export MAT_SIZE=$j
	    ...
	    mpirun -np $N_MACHINES --map-by node -mca btl self,sm,tcp bin/${FILENAME}.o
    done
done
