...
int main(int argc, char *argv[])
{
    ...
    double start_time, end_time, end_r0, global_end_time, tcomp1 = tcomp2 = tcomp3 = 99999999;
    int rank;
    MPI_Status status;
        ...
        if (rank == 0)
        {
            ...
            for (int i = 1; i < M_SIZE - 1; i += block_side)
            {
                for (int j = 1; j < M_SIZE - 1; j += block_side, mach++)
                {
                    ...
                    tcomp1 = max(end_r0-start_time, tcomp1);
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                tcomp2 = max(tcomp2,receive[block_size*block_size+2]);//verificar que é o fim do array
        ...
        else
        {
            ...
            while (!done)
            {
                ...
                if (send[0] != -1)
                {
                    ...
                    receive[block_side*block_side] = end_rx-start_rx;
